...
public class IATowa {
    ...
    void toursDeJeu() throws IOException {
        ...
        int nbToursJeu = 1;
        // indique si le joueur courant est le noir, ou pas. Change à chaque 
        // tour de jeu.
    ...
    String jouer(Case[][] plateau, int nbToursJeu) throws IOException {
        ...
        String[] actionsPossibles = Utils.nettoyerTableau(
                joueurTowa.actionsPossibles(plateau, estNoir, 5));
        if (actionsPossibles.length > 0) {
            Random r = new Random();
            int indiceAleatoire = r.nextInt(actionsPossibles.length);
            actionJouee = enleverNbPions(actionsPossibles[indiceAleatoire]);
        } else {
    ...
    /**
     * Retourne l'action sans les nombres de pions. Par exemple, sur "PfL,4,5",
     * cela renvoit "PfL".
     *

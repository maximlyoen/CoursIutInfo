...
class Plateau {
    ...
    int nbPlateformesLigne(int ligne) {
        ...
        int indiceTemporaire=0;
        int nbPlateformes = 0;
        int premierPlateforme=0;
        while(premierPlateforme!=-1 && indiceTemporaire<texteLigne.length()){
            premierPlateforme=texteLigne.indexOf('=',indiceTemporaire);
            if(premierPlateforme!=-1){
                nbPlateformes++;
                int p=premierPlateforme+1;
                boolean finPlateforme=false;
                indiceTemporaire=premierPlateforme;
                char presence;
                while(p<texteLigne.length() && !finPlateforme){
                    presence=texteLigne.charAt(p);
                    if(presence=='='){
                        p++;
                    }
                    else{ finPlateforme=false; }
                }
                indiceTemporaire=p;
            }
        }
    ...
    public String toString() {
        ...
        tabJoueurs.append(contourHoriz()).append(System.lineSeparator());
        
        for (int ligne = 0; ligne < hauteur; ligne++) {
            tabJoueurs.append("|");
            for (int colonne = 0; colonne < largeur; colonne++) {
                tabJoueurs.append(cases[ligne][colonne].toString());
            }
            tabJoueurs.append("|").append(System.lineSeparator());
        }
        tabJoueurs.append(contourHoriz()).append(System.lineSeparator());
        texte=tabJoueurs.toString();
        return texte;
